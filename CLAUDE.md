...
규칙을 어긴 것을 발견했을 때, 규칙을 하나 더 쓰는 것이 기본 대응이 아니다. 먼저 그 실패가 **부재**인지 **미준수**인지 가른다.

1. **같은 실패를 다루는 규칙이 이미 있는지 찾는다** — 이 CLAUDE.md, `src/*/CLAUDE.md`, `docs/README.md`, 관련 Design/ADR
2. **없었다(부재)** → 규칙을 쓴다. 단 발견된 한 스코프가 아니라 **그 실패가 성립하는 모든 스코프**에 쓴다. 좁게 쓴 규칙은 옆 스코프에서 같은 실패로 재발한다
3. **있었는데 안 지켜졌다(미준수)** → 규칙을 하나 더 쓰지 않는다. 같은 층위의 문장을 추가하는 것은 이미 실패한 수단을 반복하는 것이다. **기계 검사(CI·테스트·타입)로 전환**한다
4. **기계로 옮길 수 없으면** 규칙 옆에 "기계적 강제가 아님"을 명시한다 — 아래 "운영 모델"의 branch protection 항목이 그 형식이다. 강제된다고 착각한 채 남겨두지 않는다

전환 대상을 고를 때는 사람이 읽어야 작동하는 검사보다 **읽지 않아도 막히는 검사**를 우선한다.

### 기준을 결과에 맞춰 고치지 않는다

`src/core/CLAUDE.md`의 **스냅샷 갱신 함정**(구현 결과에 맞춰 테스트 기대값을 사후 수정)은 테스트에만 성립하는 함정이 아니다. 같은 형태가 Spec의 계약, Design의 아키텍처 결정, 성능 기준선에도 성립한다.

구현이 문서화된 기준을 위반한 것을 발견하면:

1. 위반 사실을 **먼저 보고**한다 (발견 → 보고 → 결정 → 수정)
2. 기준을 유지할지 바꿀지는 **위반과 분리해서** 결정한다. "구현이 이미 이러니까"는 기준 변경의 근거가 아니다
3. 기준을 바꾸기로 했다면 그 판단 근거를 기록한다 — 기준이 조용히 구현 쪽으로 이동한 이력은 나중에 복원할 수 없다

이 함정이 반복되면 위반이 새 기준이 된다 — [normalization of deviance](https://en.wikipedia.org/wiki/Normalization_of_deviance)(Diane Vaughan, _The Challenger Launch Decision_)로 불리는 실패 유형이다.
