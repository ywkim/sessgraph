...
- `--json`: 선택. 사람이 읽는 표 대신 기계가 읽는 JSON을 stdout에 출력. 봉투 형태와 오류 코드는 [기계 판독 출력 규약 Spec](20260903-1218-machine-readable-output.spec.md)이 정의한다 (`result`에 `IndexResult`가 그대로 들어간다)
