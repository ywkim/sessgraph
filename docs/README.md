...
### 4️⃣ 임의 절차 나열보다 업계 표준 용어

Design/Spec/ADR은 기술 용어가 필수다(2️⃣). 그 용어를 고를 때, 의미가 이미 합의된 **업계 표준 용어**(TDD, 골든 마스터 테스트, race condition 등)를 임의로 나열한 절차보다 우선한다 — 표준 용어는 매번 재정의할 필요 없이 같은 뜻을 재사용하게 해준다.

- 표준 용어가 이 리포 특유의 디테일(예: "픽스처는 합성해도 된다", "기대값은 사람이 계산한다")까지 함의하지 않으면, 그 디테일은 용어 옆에 각주나 하위 항목으로 구체적으로 남긴다. 용어만 던지고 그치면 에이전트가 표준 정의만으로 세부사항을 잘못 추론할 위험이 있다
- 어떤 용어가 실제로 업계에서 통용되는 정의인지 확신이 없으면 "확인 안 됨"으로 명시한다. 근거 없이 "표준"이라고 단정하지 않는다
